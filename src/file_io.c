...
void editorOpen(struct editorConfig* editor, char* filename)
{
    ...
    size_t linecap = 0;
    ssize_t linelen;
    while ((linelen = getline(&line, &linecap, filep)) !=
           -1)
    {
        while (linelen > 0 && (line[linelen - 1] == '\n' ||
                               line[linelen - 1] == '\r'))
            linelen--;
        editorInsertRow(editor, editor->numrows, line,
                        linelen);
    }
    free(line);
    fclose(filep);
    editor->num_width =
        editorGetNumLen(editor->numrows - 1);
    editor->dirty = 0;
}
...
void editorSave(struct editorConfig* editor)
{
    ...
    {
        editor->filename =
            editorPrompt(editor, "Save as: %s", NULL);
        if (editor->filename == NULL)
        {
        ...
        if (ftruncate(fd, len) != -1)
        {
            if (write(fd, buf, len) == len)
            {
                ...
                editorSetStatusMessage(
                    editor, "%d bytes written to disk",
                    len);
                return;
            }
        }
