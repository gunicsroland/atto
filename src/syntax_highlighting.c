...
void editorUpdateSyntax(struct editorConfig* editor,
                        erow* row)
{
    ...
    const char* scs =
        editor->syntax->singleline_comment_start;
    const char* mcs =
        editor->syntax->multiline_comment_start;
    ...
    int in_comment =
        (row->idx > 0 &&
         editor->row[row->idx - 1].hl_open_comment);

    int row_index = 0;
    while (row_index < row->rsize)
    {
        char current_char = row->render[row_index];
        unsigned char prev_hl = (row_index > 0)
                                    ? row->hl[row_index - 1]
                                    : HL_NORMAL;
        char prev_c = (row_index > 0)
                          ? row->render[row_index - 1]
                          : '\0';

        if (scs_len && !in_string && !in_comment)
        {
            if (!strncmp(&row->render[row_index], scs,
                         scs_len))
            {
                memset(&row->hl[row_index], HL_COMMENT,
                       row->rsize - row_index);
                break;
            }
        }
        ...
        if (mcs_len && mce_len && !in_string)
        {
            if (in_comment)
            {
                ...
                if (!strncmp(&row->render[row_index], mce,
                             mce_len))
                {
                    memset(&row->hl[row_index],
                           HL_MLCOMMENT, mce_len);
                    row_index += mce_len;
                    in_comment = 0;
                    prev_sep = 1;
                }
            ...
            else if (!strncmp(&row->render[row_index], mcs,
                              mcs_len))
            {
                memset(&row->hl[row_index], HL_MLCOMMENT,
                       mcs_len);
                row_index += mcs_len;
                in_comment = 1;
                continue;
        ...
        if (editor->syntax->flags & HL_HIGHLIGHT_NUMBERS)
        {
            if ((isdigit(current_char) &&
                ...
                (current_char == '.' &&
                 prev_hl == HL_NUMBER) ||
                (current_char == 'x' &&
                 prev_hl == HL_NUMBER && prev_c == '0') ||
                (current_char == 'b' &&
                 prev_hl == HL_NUMBER && prev_c == '0') ||
                (is_hexa_digit(current_char) &&
                 prev_hl == HL_NUMBER))
            {
        ...
        if (prev_sep)
        {
            ...
            for (keywords_index = 0;
                 keywords[keywords_index]; keywords_index++)
            {
                size_t klen =
                    strlen(keywords[keywords_index]);
                int kw2 =
                    keywords[keywords_index][klen - 1] ==
                    '|';
                if (kw2)
                {
                    klen--;
                ...
                if (!strncmp(&row->render[row_index],
                             keywords[keywords_index],
                             klen) &&
                    is_separator(
                        row->render[row_index + klen]))
                {
                    memset(&row->hl[row_index],
    ...
    struct editorConfig* editor)
{
    ...
    for (unsigned int syntax_index = 0;
         syntax_index < hldb_entries; syntax_index++)
    {
        ...
        {
            int is_ext =
                (syntax_p->filematch[language_type_index]
                                    [0] == '.');
            if ((is_ext && ext &&
                 !strcmp(ext, syntax_p->filematch
                                  [language_type_index])) ||
                (!is_ext &&
                 strstr(
                     editor->filename,
                     syntax_p
                         ->filematch[language_type_index])))
            {
                editor->syntax = syntax_p;
                return;

                int filerow = 0;
                for (filerow = 0; filerow < editor->numrows;
                     filerow++)
                {
                    editorUpdateSyntax(
                        editor, &editor->row[filerow]);
                }
            }
            language_type_index++;
        }
